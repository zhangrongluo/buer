    ...
    for group in dataset_group_cons:
        ...
        for _ in range(train_times):
            try:
                ...
                m_name = random.choice(['simple_dense', 'resnet_src', 'resnet_optimized'])
                ...
                else:  # simple dense model
                    model = get_simple_dense_model(input_dim=input_dim, dropout_rate=drop_rate)
                params = model.count_params()
                model_name = f'{model_dir}/{name_prefix}_{now}_{params}_{m_name}.keras'
                callbacks = [
                ...
                file_name = files[0]
                if 'mae' in file_name:  # 如果文件名中已经包含了mae，说明已经重命名过
                    continue
                new_suffix = f'_loss{min_loss:.4f}_mae{min_mae:.4f}.keras'
                new_file_name = file_name.replace('.keras', new_suffix)
                os.rename(f'{model_dir}/{file_name}', f'{model_dir}/{new_file_name}')
